...
class changePasswordDialog(QDialog):
    ...
    def changePasswordButtonClicked(self):
        ...
        db.open()
        query = QSqlQuery()
        sql = "SELECT * FROM User WHERE StudentName='%s'" % studentId
        query.exec_(sql)
        # 如果用户不存在
        if (not query.next()):
        ...
        sql = "SELECT * FROM User WHERE Password='%s' AND StudentName='%s'" %(md5password,studentId)
        query.exec_(sql)
        if (not query.next()):
        ...
        md5password = hl.hexdigest()
        sql="UPDATE User SET Password='%s' WHERE StudentName='%s'"%(md5password,studentId)
        query.exec_(sql)
        db.commit()
